...
it('hides the add button when viewer cannot add member', async () => {
    ...
    season.mockImplementationOnce((_, { id }) => ({
        id,
        viewerCanAddMember: false
    }))
    const api = setup.render()
    await waitFor(() => expect(season).toHaveBeenCalled())
